-- ============================================================================
-- MIGRATION: Adicionar campos logísticos aos produtos e detalhes de frete aos pedidos
-- ============================================================================

begin;

-- 1. Adicionar colunas de peso e dimensões na tabela produtos
alter table public.produtos
  add column if not exists peso_kg numeric(6,3) default 0.300,
  add column if not exists altura_cm numeric(6,2) default 4.00,
  add column if not exists largura_cm numeric(6,2) default 20.00,
  add column if not exists comprimento_cm numeric(6,2) default 25.00;

-- Garantir checks de valores positivos
alter table public.produtos
  drop constraint if exists produtos_peso_kg_check,
  drop constraint if exists produtos_altura_cm_check,
  drop constraint if exists produtos_largura_cm_check,
  drop constraint if exists produtos_comprimento_cm_check;

alter table public.produtos
  add constraint produtos_peso_kg_check check (peso_kg > 0),
  add constraint produtos_altura_cm_check check (altura_cm > 0),
  add constraint produtos_largura_cm_check check (largura_cm > 0),
  add constraint produtos_comprimento_cm_check check (comprimento_cm > 0);

-- 2. Adicionar colunas de detalhes do frete na tabela pedidos
alter table public.pedidos
  add column if not exists servico_frete text,
  add column if not exists transportadora_frete text,
  add column if not exists prazo_frete text;

-- 3. Atualizar função de persistência de entrega no checkout para salvar detalhes do frete
